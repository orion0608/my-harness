...
| | `/git-current-branch-info` 命令 | 命令区分 worktree 权威过程文档路径与主检出 `docs/<branch-id>/` 归档状态 |
| | `/git-list-branch-all` 命令 | 未 accept 分支在绑定 worktree 内检查 `docs/<branch-name>/` |
...
| | `priority-resolution` 规则 | 浏览器优先级与 MCP 工具序互补；冲突时流程以 Superpowers 为准 |
