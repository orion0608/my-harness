...
过程文档（权威，worktree）：<worktree-path>/docs/<branch-id>/ — ✅ 存在 / ❌ 不存在
  requirement.md   ✅ / ❌
  design.md        ✅ / ❌
  plan.md          ✅ / ❌
  devlog.md        ✅ / ❌
  accept-log.md    ✅ / ❌
主检出归档（docs/<branch-id>/）：✅ 存在 / ❌ 不存在 / ⬜ 不适用（main）
阶段推断：<design / plan / implement / review / accept>
```

**文档路径**（与 `doc-project-structure` 一致）：
- `<branch-id>` = 当前分支完整名称
- 功能分支且 worktree 已绑定：扫描 `{worktree}/docs/<branch-id>/` 作为权威过程文档
- `main` / `master` 主检出：过程文档行填「不适用」；可报告主检出 `docs/<branch-id>/` 是否已有历史归档目录
