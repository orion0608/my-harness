...
```
project-root/                                     ← 主检出（main）
├── README.md / CHANGELOG.md / version.md         ← 全局跟踪文档（Git 跟踪）
├── docs/
│   ├── ARCHITECTURE.md / API.md / DATABASE.md  ← 全局参考（Git 跟踪）
│   └── <branch-id>/                              ← 合入后归档备份（不跟踪）
└── .worktrees/<branch-slug>/                     ← 功能分支 worktree
    └── docs/
        ├── superpowers/                          ← Superpowers 输出（不跟踪）
        └── <branch-id>/                          ← 开发期权威过程文档（不跟踪）
            ├── requirement.md … demo/            （必写/选写清单见 doc-project-structure）
```

**核心理念**：

- **分支 + worktree**：功能分支须在独立 git worktree 中开发（`main` 仅驻留主检出）；详见 `doc-project-structure`「强制约束 — 分支与 worktree 开发」
- **过程文档**：开发期在绑定 worktree 内维护；合入时复制到主检出 `docs/<branch-id>/` 作磁盘归档（不进 Git）；详见 `doc-project-structure`
...
| 命令                  | 用途                                      |
| ------------------- | --------------------------------------- |
| `/proc-run`         | 检测技术栈，启动开发服务；含 Web 时在 IDE 浏览器打开页面      |
| `/proc-shutdown`    | 关闭当前项目、当前分支的运行中服务（工具级走 go-lifecycle 注册表） |
| `/proc-list`        | 列出当前项目全部运行中服务实例                         |
| `/proc-list-branch` | 列出当前项目、当前分支的运行中服务实例                     |
