...
Agent 在会话中主动识别有意义的问题，向用户提议记录。无需手动触发命令，体现 Harness "薄触发器" 理念。
...
| 命令 | 用途 |
|------|------|
| `/run` | 检测技术栈，启动开发服务；Web 访问展示遵循 `priority-resolution` / `preview-html` 并用浏览器验证 |
...
| 命令 | 用途 |
|------|------|
| `/myharness-test-project <path>` | 激活 `test-project/` 下子项目的逻辑工作区，开始 Harness 验证开发（`<path>` 必填，须在 `test-project/` 下；物理工作区保持为 `my_harness`） |
...
| `/git-current-branch-info` | 查看当前分支完整信息（提交、文档、阶段推断） |
...
| `/git-current-branch-delete` | 删除当前分支（main 禁止删除，打印信息后二次确认） |
