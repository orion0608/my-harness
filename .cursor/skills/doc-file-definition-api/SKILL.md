...
错误类型对齐 `docs/ARCHITECTURE.md`「接口边界」或分支 `design.md`「架构设计」中的接口契约规定，分 5 类。每个端点的错误响应必须能映射到下表之一。
...
- 必须涵盖：**错误模型**（对齐 ARCHITECTURE / `design.md`「架构设计」中的 5 类约定）、**幂等性约定**、**版本与弃用**、分页、错误响应格式、时间格式
...
- 错误类型必须可映射到 ARCHITECTURE「接口边界」/ `design.md`「架构设计」接口契约规定的 5 类
...
| 认证方式变更 | "概述" + "认证" |
| 通用约定变更 | "通用约定" |
| ARCHITECTURE「接口边界」/ `design.md`「架构设计」接口契约（错误模型 / 版本策略 / 幂等约定）变更 | "通用约定"对应小节 |

## 与其他文档的关系

- 端点契约具体记录在本文档；**架构层接口决策**（错误模型分类、版本策略、幂等约定、跨进程通信方式）记录在 `docs/ARCHITECTURE.md`「接口边界」及分支 `design.md`「架构设计」
- 本文档「通用约定」必须与上述架构文档中的接口契约保持一致；冲突时以 `docs/ARCHITECTURE.md` 及合入前的分支 `design.md`「架构设计」为准
