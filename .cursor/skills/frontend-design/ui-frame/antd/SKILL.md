...
name: antd
description: >
  Ant Design (antd) sub-skill under frontend-design. Loaded when frontend-design
  selects antd for the project. Covers writing antd components, debugging,
  querying APIs/props/tokens/demos, version migration, and project analysis
  via @ant-design/cli.
allowed-tools:
  - Bash(antd *)
  - Bash(antd bug*)
...
| Flag | Purpose |
|---|---|
| `--format ` | Output format: `json`, `text`, or `markdown` (agents should prefer `json`) |
